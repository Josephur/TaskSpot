...
| Popup never appears; journal: `Property value set multiple times` | A second handler for a signal the root object already declares (#25: a new `onVisibleChanged` added near the top while one existed at the bottom of the same object) | Grep the root's existing handlers before adding one — `grep -nE "^    on[A-Z]" SearchPopup.qml` — and fold the new logic into the existing handler. Neither `cmake --build` (qmlcachegen compiles it) nor `qmllint` (silently passes, it cannot resolve the Plasma imports) catches this; only hovering does |
